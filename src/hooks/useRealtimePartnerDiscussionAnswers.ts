    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          console.log('[Realtime] Partner discussion answer change detected:', payload.eventType)
          debouncedInvalidate()
        }
      )
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') {
          console.log(`[Realtime] Partner discussions channel subscribed for user ${user.id}`)
        } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
          console.debug(`[Realtime] Partner discussions channel status: ${status} for user ${user.id}`)
        }
      })
